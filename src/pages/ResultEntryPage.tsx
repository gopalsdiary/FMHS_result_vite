...
import { ADMIN_RESULT_ENTRY_PATH, TEACHER_LOGIN_PATH, isAdminEmail } from '@/utils/userAccess'
...
export default function ResultEntryPage() {
  ...
  useEffect(() => {
    supabase.auth.getUser().then(({ data: { user } }) => {
      if (!user) {
        sessionStorage.setItem('redirectUrl', '/result-entry')
        navigate(TEACHER_LOGIN_PATH, { replace: true })
        return
      }
      const owner = user.email ?? user.id ?? ''
      if (isAdminEmail(owner)) {
        navigate(ADMIN_RESULT_ENTRY_PATH, { replace: true })
        return
      }
      setUserEmail(owner)
      loadSubjectsFromDB(owner)
    })
  }, [navigate])
  ...
  return (
    <div style={{ fontFamily: 'var(--font-family)', background: '#f6f8fa', minHeight: '100vh' }}>
      <div style={{ background: '#0d1117', color: '#fff', padding: isMobile ? '12px 14px' : '12px 20px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '12px', flexWrap: 'wrap' }}>
            ...
            <table key={tableVersion} style={tableStyle}>
              <thead>
                <tr style={{ background: '#f0f3f6' }}>
                  ...
                  {fields.map(field => <th key={field.key} style={{ border: '1px solid #d0d7de', padding: headerPadding, textAlign: 'center', fontSize: isMobile ? '9px' : '13px', lineHeight: 1.1 }}>{compactLabel(field.label)}</th>)}
                  {comps.Total && <th style={{ border: '1px solid #d0d7de', padding: headerPadding, textAlign: 'center', background: '#f0f9ff', fontSize: isMobile ? '9px' : '13px', lineHeight: 1.1 }}>{isMobile ? 'Tot' : 'Total'}</th>}
                  {comps.GPA && <th style={{ border: '1px solid #d0d7de', padding: headerPadding, textAlign: 'center', background: '#f0f9ff', fontSize: isMobile ? '9px' : '13px', lineHeight: 1.1 }}>GPA</th>}
                  <th style={{ border: '1px solid #d0d7de', padding: headerPadding, textAlign: 'center', fontSize: isMobile ? '9px' : '13px', lineHeight: 1.1 }}>Action</th>
                </tr>
              </thead>
              <tbody>
                {students.map((row, ri) => {
                  ...
                  return (
                    <tr key={iid} style={{ background: ri % 2 === 0 ? '#fff' : '#f9fafb' }}>
                      ...
                      {fields.map(field => (
                        <td key={field.key} style={{ border: '1px solid #e1e4e8', padding: isMobile ? '2px' : '2px' }}>
                          <input
                            type="number"
                            inputMode="numeric"
                      ...
                      {comps.Total && <td style={{ border: '1px solid #e1e4e8', padding: cellPadding, textAlign: 'center', background: '#f0f9ff', fontWeight: 600, whiteSpace: 'nowrap', fontSize: isMobile ? '10px' : '13px' }}>
                        {getTotalDisplayValue(row)}
                      </td>}
                      {comps.GPA && <td style={{ border: '1px solid #e1e4e8', padding: cellPadding, textAlign: 'center', background: '#f0f9ff', fontWeight: 600, color: row[comps.GPA] === 'F' ? '#d73a49' : '#0366d6', whiteSpace: 'nowrap', fontSize: isMobile ? '10px' : '13px' }}>
