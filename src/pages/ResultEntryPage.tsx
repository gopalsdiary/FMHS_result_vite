...
export default function ResultEntryPage() {
  ...
  async function loadSubjectsFromDB(email: string) {
    ...
    // Detect subject columns from exam_ann25 (columns have '*' prefix)
  ...
  return (
    <div style={{ fontFamily: 'var(--font-family)', background: '#f6f8fa', minHeight: '100vh' }}>
      <div style={{ background: '#0d1117', color: '#fff', padding: '12px 20px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        <h1 style={{ margin: 0, fontSize: '1.1rem', fontWeight: 700 }}>Result Entry Page</h1>
        <span style={{ fontSize: '12px', color: '#aaa' }}>{userEmail}</span>
      </div>
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse', background: '#fff', fontSize: '13px' }}>
              <thead>
                <tr style={{ background: '#f0f3f6' }}>
                  <th style={{ border: '1px solid #d0d7de', padding: '8px', textAlign: 'center' }}>IID</th>
                  <th style={{ border: '1px solid #d0d7de', padding: '8px' }}>Name</th>
                  <th style={{ border: '1px solid #d0d7de', padding: '8px', textAlign: 'center' }}>Roll</th>
                  {comps.CQ && <th style={{ border: '1px solid #d0d7de', padding: '8px', textAlign: 'center' }}>CQ</th>}
                  {comps.MCQ && <th style={{ border: '1px solid #d0d7de', padding: '8px', textAlign: 'center' }}>MCQ</th>}
                  {comps.Practical && <th style={{ border: '1px solid #d0d7de', padding: '8px', textAlign: 'center' }}>Practical</th>}
                  {comps.Total && <th style={{ border: '1px solid #d0d7de', padding: '8px', textAlign: 'center', background: '#f0f9ff' }}>Total</th>}
                  {comps.GPA && <th style={{ border: '1px solid #d0d7de', padding: '8px', textAlign: 'center', background: '#f0f9ff' }}>GPA</th>}
                </tr>
              </thead>
              <tbody>
                {students.map((row, ri) => {
                  ...
                  return (
                    <tr key={iid} style={{ background: ri % 2 === 0 ? '#fff' : '#f9fafb' }}>
                      ...
                      {comps.CQ && <td style={{ border: '1px solid #e1e4e8', padding: '2px' }}>
                        <input type="number" defaultValue={row[comps.CQ!] !== null && row[comps.CQ!] !== undefined ? String(row[comps.CQ!]) : ''} onChange={e => handleEdit(iid, comps.CQ!, e.target.value)} style={{ width: '60px', padding: '4px', border: '1px solid #ccc', borderRadius: '3px', textAlign: 'center', fontSize: '12px' }} />
                      </td>}
                      {comps.MCQ && <td style={{ border: '1px solid #e1e4e8', padding: '2px' }}>
                        <input type="number" defaultValue={row[comps.MCQ!] !== null && row[comps.MCQ!] !== undefined ? String(row[comps.MCQ!]) : ''} onChange={e => handleEdit(iid, comps.MCQ!, e.target.value)} style={{ width: '60px', padding: '4px', border: '1px solid #ccc', borderRadius: '3px', textAlign: 'center', fontSize: '12px' }} />
                      </td>}
                      {comps.Practical && <td style={{ border: '1px solid #e1e4e8', padding: '2px' }}>
                        <input type="number" defaultValue={row[comps.Practical!] !== null && row[comps.Practical!] !== undefined ? String(row[comps.Practical!]) : ''} onChange={e => handleEdit(iid, comps.Practical!, e.target.value)} style={{ width: '60px', padding: '4px', border: '1px solid #ccc', borderRadius: '3px', textAlign: 'center', fontSize: '12px' }} />
                      </td>}
                      {comps.Total && <td style={{ border: '1px solid #e1e4e8', padding: '4px 8px', textAlign: 'center', background: '#f0f9ff', fontWeight: 600 }}>
                        {row[comps.Total!] !== null && row[comps.Total!] !== undefined ? String(row[comps.Total!]) : ''}
                      </td>}
                      {comps.GPA && <td style={{ border: '1px solid #e1e4e8', padding: '4px 8px', textAlign: 'center', background: '#f0f9ff', fontWeight: 600, color: row[comps.GPA!] === 'F' ? '#d73a49' : '#0366d6' }}>
                        {row[comps.GPA!] !== null && row[comps.GPA!] !== undefined ? String(row[comps.GPA!]) : ''}
                      </td>}
                    </tr>
                  )
                })}
