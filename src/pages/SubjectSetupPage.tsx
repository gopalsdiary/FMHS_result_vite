...
export default function SubjectSetupPage() {
  ...
  async function detectSubjects() {
    ...
    if (!data?.length) { setStatus('No data found in exam_ann25'); setLoading(false); return }
        ...
        <div style={{ maxWidth: '1000px', margin: '0 auto' }}>
          <div className="card" style={{ marginBottom: '16px', background: '#fff8e1', border: '1px solid #f9a825' }}>
            <p style={{ margin: 0, fontSize: '14px', color: '#5d4037' }}>
              ℹ️ Subject columns are auto-detected from the <strong>exam_ann25</strong> table. Columns follow the pattern <code>*SubjectName_CQ</code>, <code>*SubjectName_MCQ</code>, etc. To add or remove subjects, update the database schema via SQL.
            </p>
          </div>
          ...
          {!loading && subjects.length === 0 && !status && (
            <div className="card" style={{ textAlign: 'center', color: '#6a737d', padding: '40px' }}>
              No subject columns detected in exam_ann25.
            </div>
          )}
        </div>
      )}
