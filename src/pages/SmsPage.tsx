...
export default function SmsPage() {
  ...
  function buildShortSms(s: StudentFull): string {
    const header = selectedExam ? selectedExam.exam_name : 'Exam'
    const year = selectedExam ? selectedExam.year : ''
    let smsText = `Dear ${s.student_name_en}, your ${header} ${year} total mark is ${s.total_mark || 0}. Headmaster, Feni Model High School. https://modelresult.netlify.app/sms/ann25.html?IID=${s.iid}`
  ...
  function exportCsv() {
    ...
    const a = document.createElement('a'); a.href = url; a.download = `sms_short_export_${gridClass}.csv`; a.click()
  ...
  const rawSegments = activeStudents.reduce((sum, s) => {
    const segs = getSegments(buildShortSms(s))
  ...
  return (
    <PageShell title="Short SMS Generator">
      {() => (
